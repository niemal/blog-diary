...
export function getBlogPosts() {
    ...
    fileNames.forEach(name => {
        ...
        if (lines[0].startsWith('banner:!')) {
            ...
            banner = '/blog/_next/image?url=%2Fbanners%2F'
...
export function getPostData(id) {
    ...
    if (lines[0].startsWith('banner:!')) {
        ...
        banner = '/blog/_next/image?url=%2Fblog%2Fbanners%2F'
            + lines.shift().split('banner:')[1]
            + '&w=1920&q=100'; // local cdn
    }
            ...
            if (lang !== null) {
                ...
                if (tmp.indexOf(codeLang) === -1 || val === '') {
                    ...
                    content = content.replaceAll(
                        `<code${tmp}</code>`,
                        `<code class="hljs ${styles.oneLineCode}">${tmp.substring(1, tmp.length)}</code>`
                    );
                    continue;
                }
                ...
                content = content.replaceAll(
                    `<code${tmp}</code>`,
                    `<code class="hljs">${hljs.highlight(val, {language: codeLang}).value}</code>`
                );
            }
    ...
    content = content.replaceAll('<p', `<p class="paragraph text-md lg:text-xl font-light text-white mb-4 mt-6" `);
    ...
    content = content.replaceAll('<ol', `<ol class="text-md lg:text-xl font-light text-white lg:mb-1 mt-4" `);
    content = content.replaceAll('<li', `<li class="pl-2 mt-2" `);
    content = content.replaceAll('<ul', `<ul class="text-md lg:text-xl font-light text-white lg:mb-1 mt-4" `);
    content = content.replaceAll('<code class="hljs"', `<pre><code class="hljs ${styles.code}"`);
    content = content.replaceAll('<code>', `<code class="hljs ${styles.oneLineCode}">`);
